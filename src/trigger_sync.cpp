...
//  {
    ...
    if (match_events_)
    {
      // Add message to approximate syncroniser
      ...
      addMsgToApproximateSync(event_msg_ptr,0);
    }
...
void TriggerSync::trigger_msg_cb_(const trigger_sync::Event::ConstPtr& event_msg)
{
  ...
  addMsgToApproximateSync(event_msg, 1);
...
//      ROS_DEBUG_STREAM("Added device time " << event_msg->device_time << " to queue (0) with "  << event_msg->device_clock_id);
  ...
  {
    boost::shared_ptr<Sync2> current_sync;
    ...
    message_matching_map[event_msg->device_clock_id]->add<1>(p);
...
//    exit(1);
  ...
  if ( remote_message->event.local_receive_time <=  local_message->event.local_receive_time )
  {
    ...
    // Other message as earlier recieve time. Replace recieve time and add to estimators
    trigger_sync::Event event_msg;
    ...
    ROS_INFO_STREAM(" Match( Match dela= "  << NUM_FORMAT <<(local_message->header.stamp - remote_message->header.stamp).toSec()*1000.0 << "ms, receive time delta = " << NUM_FORMAT  <<(local_message->event.local_receive_time - remote_message->event.local_receive_time).toSec()*1000.0 << "ms ) - add to buffer - " << min_receive_delta << "(" << 500/min_receive_delta << "Hz" );

    updateMsg(event_msg);
  } else {
    ROS_INFO_STREAM(" Match( Match dela= " << NUM_FORMAT << (local_message->header.stamp - remote_message->header.stamp).toSec()*1000.0 << "ms, receive time delta = " << NUM_FORMAT  <<(local_message->event.local_receive_time - remote_message->event.local_receive_time).toSec()*1000.0 << "ms ) - not adding to buffer");
...
bool TriggerSync::bufferMsgs(trigger_sync::Event& event_msg)
{
  ...
  if ( event_msg.device_time <= estimator_min_time_)
  {
  ...
  // Check through vector for duplicate device times (TICSYNC needs an ever increaseing device time)
  ...
  for(uint i = 0; i < events_.size(); i++)
  {
    ...
    else if (events_[i].device_time == event_msg.device_time)
    {
      ...
      // Set local_request_time to maximum possible time
      ...
      // Set local_receive_time tom minimum posible time
